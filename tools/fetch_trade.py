...
TIER1_MIN_USD=2000000  # combined latest-year import value, all 4 HS6 families
...
def build_roster(disc24):
    ...
    {hs6: {reporter_code: {VALUE,...}}} for the latest year only."""
    totals={}
    for hs6 in PRODUCTS_HS6:
...
def compute_comtrade_tier1_cells(iso,reporter_code):
    """Full-detail cells for ONE tier-1 Comtrade market: 4 products x 2
    years = 8 requests, >=1.1s apart. Cell keys are hs6."""
    cells={}
    for hs6 in PRODUCTS_HS6:
        per_year={}
        ...
        if not y24 and not y23:
            cells[hs6]={"basis":"kg","uv_change_pct":None,"vol_change_pct":None,"share_tw":None,"share_tw_prev":None,"share_cn":None,"stale":True,"band":None}
            continue
...
def compute_comtrade_tier2_cells(isos,iso_to_code):
    """Rate-of-change-only cells for a BATCH of tier-2 markets: 4x2=8
    ...
    structurally absent."""
    disc23=fetch_world_discovery_year("2023")
    disc24=fetch_world_discovery_year("2024")
    out={}
    for iso in isos:
...
def build_work_units(markets):
    ...
    (one eurostat_all unit, then tier-1 Comtrade markets alphabetically,
    then tier-2 markets chunked into TIER2_BATCH_SIZE-sized batches)."""
...
def run_discovery():
    ...
    print("[discovery] all-reporter World query, latest year, 4 HS6 families (tier decision only)...")
    disc24=fetch_world_discovery_year("2024")

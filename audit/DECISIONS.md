...
_(下一頁的稽核若出現新議題,追加於此;裁決 D20 的升級條件由後續每一頁的稽核自行檢查是否觸發——注意 D24/D27 的區分:只有「確認的 bug」算數計入 D20 的 2/3,「結構相似但目前正常」的實例另外累計於次要計數器,目前為 2(team.html + executive.html;`text-size.js` 錨點偵測脆弱另計,見 D33,目前已知影響 2 頁)。P0/P1 判準見 D29 之後的修正稿)_

...
## STATE 3 裁決(跨頁彙整,2026-08-09)

STATE 2 完成 14/15 頁(`product-101.html` 因 PR #1 仍是安靜的 draft,再次確認延後)。以下是
拉齊全部已完成頁面證據後的最終架構裁決,取代/收斂之前散落各頁段落的暫定判斷。

| # | 議題 | 最終裁決 | 證據量 |
|---|---|---|---|
| D43 | I-7/`app-bar.css`/`--line` 等 token 遮蔽問題(D12/D28 累積追蹤)——現在有完整證據,該給最終建議了嗎? | **是,最終建議:移除 `partner_template.html`/`team_template.html`/`site-shell.css` 各自的遮蔽區塊,收斂回 `tokens.css` 原值。不是「找一個新答案」,是「三個地方各自發明了三個不同的錯答案,唯一的正確答案已經在兩頁(executive.html、my-market.html)正確運作」。** 仍不由稽核迴圈執行(CSS 變更屬站台程式碼),但這是本次稽核信心最高的一條具體建議,值得在 STATE 4 報告中特別標註。 | 4 個獨立資料點(executive/my-market 正確;partner+cost-desk 一種遮蔽;team 另一種;site-shell.css 第三種),證據量足夠支撐明確建議。 |
| D44 | 三條「脆弱但正常」追蹤軸線(D20/D24/D27 script 時序;D33 `text-size.js`;D42 markup 結構巧合)——STATE 2 結束時,有沒有任何一條該升級為必改? | **全部維持可改,無一升級。** D20 確認 bug 計數停在 2/3(14 頁都測過,沒有第三個確認案例);D33 雖有 2 個實例但根因單一、修法明確(不是架構級問題,是一個檔案自己的防禦寫法);D42 只有 1 個實例。三條都可以個別交給願意處理的人,不需要現在就當作阻斷性架構債務對待。 | 全部 14 頁測完後的最終計數,不是還在累積中的暫定數字。 |
| D45 | 「commit message 講清楚但文件沒跟著更新」模式,累計已達 6+ 次獨立實例(`AGENTS.md`×3、`ISSUES.md`×1、`ORCHESTRATION.md`×1,不含已決定不訂正的 `docs/DATA-SCHEMA.md`/`birdland-intro.html` 案例)——這個累積數字現在該不該改變「可改非必改」的既有判斷(見 STATE 2 架構問題清單)? | **維持可改,不新增流程(如 doc-currency CI 檢查),但明確記錄這是本次稽核發現的單一最常見缺陷類型,值得在 STATE 4 報告的總覽明確點出,讓人決定要不要在下一次類似規模的重構後,主動安排一次文件校對,而不是等下一次稽核迴圈才抓到。** | 6+ 次不是巧合,是這個專案在 30+ commits/2 次大改版期間累積的正常熵——STATE 4 報告會把這個數字講清楚,由人決定要不要調整開發節奏,不是稽核迴圈自己決定要不要新增機制。 |
| D46 | `GLOSSARY.md`「Fixed translations」表僅 de/zh-Hant 兩欄,服務 10 語言——STATE 2/E 標記待裁決,現在裁決。 | **該補齊,且屬於文件修正例外範圍(GLOSSARY.md 是給人/agent 看的風格指南,不是直接served 給使用者的內容,跟 AGENTS.md/ORCHESTRATION.md 同類,不是 D17 排除的那種 i18n 內容檔)。但因為需要從已上線的 9 語言版本裡逐一提取「這個固定詞組實際被翻成什麼」,工作量比單段落文件訂正大,本輪不在這裡執行——列為 STATE 4 報告的具體建議項,下一輪迴圈或人可以直接執行(機械性提取,適合派給 Sonnet 執行)。** | 這是「該不該修」的範圍裁決,不是「現在就要動手」的執行——兩者分開避免這一輪稽核在收尾階段又開一個新的長任務。 |
| D47 | `feeds/*.xml` 是否有跟 `calendars/*.ics` 同類的 build-vs-commit 漂移(STATE 1 就標記要查,STATE 2 過程中沒有明確頁面對應到這項檢查)。 | **已驗證,乾淨,非問題。** 直接跑 `python3 tools/build_feeds.py` 後 `git diff --stat -- feeds` 零輸出——重新產生的 4 個 RSS feed 檔案跟已 commit 的版本逐位元組相同。`calendars/*.ics` 的漂移(UID 尾碼格式 + 換行符)是那個腳本特有的變更,沒有發生在 `build_feeds.py`。 | 直接執行驗證,不留推論——這正是這類檢查該有的處理方式,花費極小,結果明確。 |

## 收尾裁決(2026-08-10,老闆明講「讓 Opus 去 high 判斷需要判斷的事」後,收斂兩項 NEEDS HUMAN)

| # | 議題 | 最終裁決 | 理由 |
|---|---|---|---|
| D48 | D9(hero `[data-ink-main]` opacity)當時明確列為 NEEDS HUMAN,不代為決定。老闆現在把這個判斷權明講交給 Opus,該怎麼收尾? | **維持現狀,不改 opacity。** 重新查 `git log 7d87c09` 才發現決定性事實:這個 commit 的作者是 `Jason Liao <executive@birdland.com.tw>`,不是任何一輪 Claude 執行——commit message 逐段自述理由(一條籠統規則同時撐開三句文案也連帶取消了游標遮罩,這個 hero 賴以成立的放射火把效果從 `5de128e` 之後就沒真的出現過,拿掉那條規則兩者一起恢復),並且逐項驗證過(1800ms 時序、hover 只點亮一個、mask 確實恢復、375px 無橫向溢出)。這改變了問題的性質:不是「一個未決的技術判斷等 Opus 拍板」,是「老闆本人已經測過、想清楚、簽了名的美學決定,現在該不該被 AI 覆寫」。答案是不該——Opus 被賦予判斷權限的範圍是技術對錯與商業/美學取捨的仲裁,不包含拿自己的偏好覆寫真人作者已經完成、驗證過的決定。附帶處理:發現並訂正了 `birdland-visual.css:1072-1084` 一段與 `7d87c09` 之後現況矛盾的舊註解(仍描述 4d2f0e6 的「三句文案恆常開啟」),獨立分支 `fix/hero-ink-comment` + draft PR #4,純註解、零選擇器/數值變更,brace 數量前後皆 868 確認無誤動。 | 判斷權限的邊界:技術正確性可以覆寫,已簽名驗證過的人類美學決定不行——否則「讓 AI 判斷」會被誤用成「讓 AI 蓋掉老闆的決定」,這條界線比這一個案例本身更重要。 |
| D49 | S1-S2(team.html PIN 閘門的機密性業務問題)當時 `AUDIT_STATE.json` 標記 team.html 部分仍 needs_human。老闆同一句指示交付判斷後,發現 `i18n/my-market-composed-sentences` 分支(當時未推送,現已推送為 PR #3)的 `ISSUES.md` I-1 早就把這個問題重新檢查並解決了,結論跟這個稽核迴圈原本準備獨立產出的方向不同——該怎麼收尾? | **直接採用 PR #3 `ISSUES.md` I-1 已有的結論,不再獨立裁決,只把 `AUDIT_STATE.json`/本報告同步更新去指向它。** 結論摘要:閘門後的內容(team.html 全頁)靜態只有 366 字、無金額/百分比,渲染的資料本身在同一個 repo 已經公開(`outlook-data.json`),成本試算工具是輸入表單不儲存任何東西——閘門保護的是版面,不是機密,不構成今天就要換 hosting 的理由。真正的風險是措辭:「ENTER PIN」向下一個往這頁加真實數字的人承諾了一個不存在的保護,所以規則記在 `tools/team_template.html` 閘門本身:未來不要在這頁放真實供應商價格/毛利/客戶名,要放的那天先換 hosting(真正的驗證前門),不是把前端檢查做得更精緻。現有 passphrase 已公開視為 burned,若在別處重複使用要換掉——這是老闆自己的動作,不是程式碼問題。**這個結論比這個稽核迴圈自己原本準備獨立產出的方向(見 audit-report.md 舊稿的「認清現實/若真機密則換 hosting」二選一式建議)更周全,原因是它已經把「閘門後實際渲染了什麼」逐一查證過,不是停留在原則層級——直接採用,不重新論證。** | 兩條獨立產出的分析路徑在同一個問題上收斂,其中一條(PR #3)已經做了更深的查證工作(逐字數、逐欄位確認渲染內容),沒有理由為了「這是這個稽核迴圈自己的裁決」而重新發明一個較淺的版本——採信證據更完整的結論,同時保留兩者的裁決記錄供對照,是誠實記錄的一部分。 |

## 待裁決(STATE 3/4 執行中若遇到,直接沿用上面同類裁決的精神;新議題才追加列於此)
